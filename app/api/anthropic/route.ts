// Claude/Anthropic API Route - Non-streaming version
...
export async function POST(req: Request) {
  ...
  try {
    console.log("Starting Claude API call with messages:", JSON.stringify(messages).substring(0, 100) + "...")
...
WICHTIG: Du darfst NUR dann eine endgültige Empfehlung abgeben, wenn alle vier Validierungsmethoden ein konsistentes Ergebnis liefern. Bei Inkonsistenzen musst du deine Analyse überarbeiten, bis ein stimmiges Gesamtbild entsteht.`,
    ...
    console.log("Claude API call completed, text length:", result.text.length)
    ...
    return new Response(JSON.stringify({ error: "Failed to connect to Anthropic API", details: String(error) }), {
      status: 500,
      headers: { "Content-Type": "application/json" },
    })
  }
